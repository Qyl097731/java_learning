...
如果存在只被部分类使用的函数，就可以考虑把这部分提炼成一个子类。

- 首先创建子类,创建构造函数（先简单调用父类构造函数），修改所有可以用子类替换的地方，把其改成对子类的引用。
- 之后简化子类的构造函数的参数列表，并在父类中创建对应的构造函数，新构造函数通过调用旧构造函数进行初始化（向旧构造函数传递null来代替不需要的列）
- 函数、字段下移

### 提炼超类

如果两个类存在相同函数，就可以抽取到一个父类中。

- 新建一个空白的抽象超类
- 把相同方法和字段移动到超类

    - 先字段比较简单
    - 如果子类函数有不同名字，用途相同，就改名；
    - 如果相同签名，但是函数体不同，就在超类中声明抽象函数
    - 如果不同函数体，用途相同，两个一起传到超类之后选择使用。
- 编译测试后继续观察类，继续提炼。
- 把对子类的引用改为对超类的引用 

### 塑造模板函数

有一些子类，其中相应的某些函数以相同顺序执行类似的操作，但是操作的细节上有所不同。

<b>可以把这些操作放进独立函数，保持他们相同的函数名字，然后将原函数移动到超类；最后在超类中把相同函数名作为抽象类添加进去</b>

### 以委托取代继承

如果某个子类只是用超类接口中的一部分，或者根本不需要继承而来的数据。

<b>做法：</b>在子类中新建一个字段用以保存超类；调整子类函数，令它改成委托超类，然后去掉继承。

### 以继承取代委托

在两个类之间使用委托，经常为整个接口编写许多极为简单的委托函数。

## 大型重构

### 梳理并分解继承体系

<b>场景：</b>某个继承体系同时承担了两项责任，这时候可以建立两个继承体系，并通过委托关系让一个调用另一个。

<b>做法:</b>

- 识别继承体系中的不同职责，建立二维表格分别写出这两种职责
- 判断哪种职责更重要，把重要的保存下来，另一种提炼到一个新的类B，把继承体系中的相同职责的子类变为继承B
- 在原超类中A添加对B子类的引用。

### 将过程化设计转化为对象设计

<b>场景：</b>过程化风格的代码需要改变,可以将数据变成对象，将大块的行为分成小块，并将行为移入相关对象之中。

<b>做法:</b>

- 把记录类型抽取成哑对象，只提供数据
- 把大型函数抽取到工具类，在工具类再进行重构
...
简单点说就是把不能的分支抽取成类，用策略\状态模式进行改造，但是需要保证状态转换的判断依据在整个过程都不能变。
