...
### 组合子很好，但是类更好（面向对象VS面向函数）

组合子是一种基于函数式编程思想的概念，指的是使用高阶函数组合而成的函数。它们可以接受或返回其他函数，可以被用作其他函数的参数。组合子通常被用来简化函数的嵌套和提高代码的可读性。

使用组合子可以更好地支持函数式编程的一些概念，比如柯里化（currying）和偏函数（partial function）。柯里化是指将一个多参数函数转化为一系列单参数函数的过程，偏函数则是通过固定一个或多个参数，来得到一个新的、接受较少参数的函数。使用组合子可以更容易实现这些概念，并且可以提高代码的灵活性和可复用性。

不过，使用组合子也有一些限制。由于在函数式编程中，函数一般只和输入输出有关，不依赖其他外部状态，因此使用组合子很难实现涉及状态修改和副作用的操作。这些操作需要使用其他手段来实现，比如使用状态传递或者使用特定的数据结构来保存状态。

举个例子，假设我们有两个函数f和g，分别实现如下：
...
那么我们可以通过组合这两个函数来得到一个新的函数h，它对输入的x先调用g函数，再调用f函数，实现如下：
...
在面向对象编程中，我们通常会使用类来代替函数，因为类可以封装状态和行为，通过方法来操作状态。能控制模型的复杂度，同时还能充分发挥不同类型存在共性的优势。

# 总结

- 从项目的利益相关人入手，洞悉相关人的需求，我称之为隐形需求，之后再关注功能需求，毕竟系统应该服务于人。
- 需求完善之后，再不断地迭代改进过程中，保证质量品质、可靠性、可维护性、可扩展性，在这个过程中架构自然而然会形成。
- 架构不是一尘不变，能随意复制，到处使用的框架。要根据实际需求进行认真死锁，但是在整个开发过程中，要以美丽的架构为标榜，但是始终以实现需求为核心。

将设计的研究和设计的实践分离开来是非常荒唐的。实际上，学习设计方法而不实践设计的人几乎都是失败的设计者，他们没有活力，他们丧失 了创造事物的冲动，或者说从来就没有过这种冲动。
